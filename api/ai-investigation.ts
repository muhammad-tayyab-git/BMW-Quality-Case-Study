import type { AIInvestigationEvidencePacket } from "../src/types/aiInvestigation";

const GEMINI_MODEL =
  process.env.GEMINI_MODEL || "gemini-3.5-flash-lite";

const MAX_BODY_BYTES = 500_000;

const SYSTEM_INSTRUCTIONS = `You are an engineering quality investigation assistant inside a BMW production-quality dashboard.

Your one job is to turn a Task 3 engineer-flagged defect plus a small, deterministic evidence set into a short factual investigation brief.

Rules:
- Use ONLY the supplied evidence packet.
- Never invent records, numbers, causes, suppliers, process facts, or filters.
- The application selected the evidence rows before you were called.
- Do not ask for more data and do not expand the evidence set.
- The application owns all statistical calculations.
- Do not replace or reinterpret Task 2 outlier methods.
- Distinguish observations from hypotheses.
- Never claim that a root cause is confirmed.
- Use wording such as "possible investigation topic", "may warrant checking", or "the evidence shows".
- Keep the output short.
- Maximum 2 observed patterns.
- Maximum 2 investigation topics.
- Maximum 2 recommended checks.
- Keep every text field below 180 characters.
- Use concise factual wording.
- Every item must cite one or more exact defectId values from the supplied evidenceRows.
- Never cite an ID that is not present in evidenceRows.
- Do not make a vehicle-level quality verdict.
- Do not recommend automatic rejection.
- If the evidence is insufficient, say so instead of guessing.

Return only the JSON structure requested by the response schema.`;

const AI_RESPONSE_SCHEMA = {
  type: "OBJECT",
  properties: {
    observedPatterns: {
      type: "ARRAY",
      items: {
        type: "OBJECT",
        properties: {
          text: {
            type: "STRING",
          },
          evidenceIds: {
            type: "ARRAY",
            items: {
              type: "STRING",
            },
          },
        },
        required: ["text", "evidenceIds"],
      },
    },

    investigationTopics: {
      type: "ARRAY",
      items: {
        type: "OBJECT",
        properties: {
          text: {
            type: "STRING",
          },
          evidenceIds: {
            type: "ARRAY",
            items: {
              type: "STRING",
            },
          },
        },
        required: ["text", "evidenceIds"],
      },
    },

    recommendedChecks: {
      type: "ARRAY",
      items: {
        type: "OBJECT",
        properties: {
          text: {
            type: "STRING",
          },
          evidenceIds: {
            type: "ARRAY",
            items: {
              type: "STRING",
            },
          },
        },
        required: ["text", "evidenceIds"],
      },
    },
  },

  required: [
    "observedPatterns",
    "investigationTopics",
    "recommendedChecks",
  ],
};

function jsonResponse(
  payload: unknown,
  status = 200,
): Response {
  return Response.json(payload, {
    status,
    headers: {
      "Cache-Control": "no-store",
    },
  });
}

function extractOutputText(response: unknown): string {
  if (
    typeof response !== "object" ||
    response === null
  ) {
    return "";
  }

  const data = response as {
    candidates?: Array<{
      content?: {
        parts?: Array<{
          text?: unknown;
        }>;
      };
    }>;
  };

  const parts: string[] = [];

  for (const candidate of data.candidates ?? []) {
    for (const part of candidate.content?.parts ?? []) {
      if (typeof part.text === "string") {
        parts.push(part.text);
      }
    }
  }

  return parts.join("\n");
}

function parseModelJson(text: string): unknown {
  const cleaned = text
    .trim()
    .replace(/^```json\s*/i, "")
    .replace(/^```\s*/i, "")
    .replace(/\s*```$/i, "")
    .trim();

  return JSON.parse(cleaned);
}

function sanitizeResult(
  result: unknown,
  evidenceIds: string[],
) {
  const allowed = new Set(evidenceIds);

  const cleanList = (
    value: unknown,
    maxItems: number,
  ) => {
    if (!Array.isArray(value)) {
      return [];
    }

    return value
      .filter(
        (item): item is {
          text: string;
          evidenceIds?: unknown;
        } =>
          typeof item === "object" &&
          item !== null &&
          typeof (item as { text?: unknown }).text === "string",
      )
      .slice(0, maxItems)
      .map((item) => ({
        text: item.text.trim().slice(0, 400),

        evidenceIds: Array.isArray(item.evidenceIds)
          ? [
              ...new Set(
                item.evidenceIds.filter(
                  (id): id is string =>
                    typeof id === "string" &&
                    allowed.has(id),
                ),
              ),
            ].slice(0, 6)
          : [],
      }))
      .filter(
        (item) =>
          item.text.length > 0 &&
          item.evidenceIds.length > 0,
      );
  };

  const typedResult =
    typeof result === "object" &&
    result !== null
      ? (result as {
          observedPatterns?: unknown;
          investigationTopics?: unknown;
          recommendedChecks?: unknown;
        })
      : {};

  return {
    observedPatterns: cleanList(
      typedResult.observedPatterns,
      3,
    ),

    investigationTopics: cleanList(
      typedResult.investigationTopics,
      3,
    ),

    recommendedChecks: cleanList(
      typedResult.recommendedChecks,
      2,
    ),
  };
}

async function callGemini(
  evidence: AIInvestigationEvidencePacket,
) {
  const apiKey = process.env.GEMINI_API_KEY;

  if (!apiKey) {
    throw new Error(
      "GEMINI_API_KEY is not configured on the server.",
    );
  }

  const prompt = [
    SYSTEM_INSTRUCTIONS,
    "Create a very concise investigation brief from this evidence packet.",
    "Do not repeat the evidence packet.",
    JSON.stringify(evidence),
  ].join("\n\n");

  const url =
    `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(
      GEMINI_MODEL,
    )}:generateContent`;

  const response = await fetch(url, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "x-goog-api-key": apiKey,
    },
    body: JSON.stringify({
      contents: [
        {
          role: "user",
          parts: [{ text: prompt }],
        },
      ],

      generationConfig: {
        responseMimeType: "application/json",

        responseSchema: AI_RESPONSE_SCHEMA,

        temperature: 0.1,

        maxOutputTokens: 700,
      },
    }),
  });

  const data: any = await response.json();

  if (!response.ok) {
    throw new Error(
      data?.error?.message ||
        `Gemini request failed with status ${response.status}.`,
    );
  }

  const candidate = data?.candidates?.[0];

  if (!candidate) {
    throw new Error("Gemini returned no candidate.");
  }

  // Gemini tells us when the output stopped.
  // MAX_TOKENS means the JSON may have been cut off.
  if (candidate.finishReason === "MAX_TOKENS") {
    throw new Error(
      "The AI response was truncated. Please try generating the brief again.",
    );
  }

  const outputText = extractOutputText(data);

  if (!outputText) {
    throw new Error(
      "The LLM returned no text output.",
    );
  }

  try {
    return parseModelJson(outputText);
  } catch (error) {
    console.error(
      "Invalid Gemini JSON response:",
      outputText,
    );

    throw new Error(
      "The AI returned an invalid response. Please try generating the brief again.",
    );
  }
}

export async function POST(
  request: Request,
): Promise<Response> {
  try {
    const contentLength =
      request.headers.get("content-length");

    if (
      contentLength &&
      Number(contentLength) > MAX_BODY_BYTES
    ) {
      return jsonResponse(
        {
          error: "Request body is too large.",
        },
        413,
      );
    }

    const body: unknown = await request.json();

    const evidence =
      typeof body === "object" &&
      body !== null &&
      "evidence" in body
        ? (body as {
            evidence?: AIInvestigationEvidencePacket;
          }).evidence
        : undefined;

    if (
      !evidence ||
      !Array.isArray(evidence.evidenceRows) ||
      !Array.isArray(evidence.flaggedDefectIds) ||
      evidence.flaggedDefectIds.length === 0
    ) {
      return jsonResponse(
        {
          error:
            "The request does not contain a valid flagged-evidence packet.",
        },
        400,
      );
    }

    const evidenceIds = evidence.evidenceRows
      .map((row) => row.defectId)
      .filter(
        (id): id is string =>
          typeof id === "string" &&
          id.length > 0,
      );

    const result = await callGemini(evidence);

    const sanitized = sanitizeResult(
      result,
      evidenceIds,
    );

    return jsonResponse({
      result: sanitized,
      evidence,
      model: GEMINI_MODEL,
      generatedAt: new Date().toISOString(),
    });
  } catch (error) {
    console.error(
      "Task 5 AI investigation error:",
      error,
    );

    return jsonResponse(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unexpected AI service error.",
      },
      500,
    );
  }
}